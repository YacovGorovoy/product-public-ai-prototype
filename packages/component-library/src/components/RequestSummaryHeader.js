...
export default function RequestSummaryHeader({
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 pb-4 border-b border-gray-100 mb-4">
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-2 flex-wrap">
          <Typography variant="h2" className="!mb-0 !text-2xl font-bold text-gray-900">
            {title}
          </Typography>
          {requestNumber && (
            <span className="text-gray-500 text-base font-medium">(Request-{requestNumber})</span>
          )}
          {status && (
            <Badge color={statusColor}>{status}</Badge>
          )}
        </div>
        <div className="flex items-center gap-4 text-gray-700 text-base font-medium flex-wrap">
