...
const navItems = [
  ...
  },
  {
    ...
    label: 'Bills',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="4" y="4" width="16" height="16" rx="2" /><path d="M8 8h8M8 12h8M8 16h8" /></svg>
    ),
  },
  {
    key: 'payments',
  ...
  },
  {
    ...
    label: 'Detect',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 21l-4.35-4.35" /><circle cx="11" cy="11" r="8" /></svg>
    ),
  },
  {
    key: 'integrations',
    label: 'Integrations',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M7 7h10v10H7z" /><path d="M3 3h4v4H3zM17 3h4v4h-4zM3 17h4v4H3zM17 17h4v4h-4z" /></svg>
    ),
  },
  {
    key: 'reports',
...
export default function Sidebar({ active = 'home', onNavClick }) {
  ...
  return (
    <aside className={`sidebar fixed left-0 top-0 h-screen z-50 ${expanded ? 'w-48' : 'w-16'} transition-all duration-200 flex flex-col items-center py-4 space-y-6 text-white bg-[#2c3e50] justify-between`}>
        ...
        <nav className="flex flex-col items-center space-y-2 flex-grow mt-4 w-full">
          {navItems.map(item => (
            ...
            >
              ...
              {item.key === 'approvals' && (
                <span className="absolute top-1 right-1 bg-red-500 text-white text-[10px] rounded-full px-1.5 py-0.5">3</span>
              )}
            </button>
          ))}
        </nav>
